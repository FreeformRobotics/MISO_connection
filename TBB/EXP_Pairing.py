...
class EXP_Pairing():
    def __init__(self, A, B, b, matrix_type=''):
        self.A=A
        self.B=B
        ...
        data_input = open('../../data/results/CNL_Pairing/'+ self.matrix_type + str(self.b) + '.txt', 'rb')
        result_cnl = pickle.load(data_input)
        data_input.close()
        self.UPPER=result_cnl[0]
    ...
    def generate(self, parent_node_ID, printer=True):
        ...
        if len(parent_node.mask_a) == self.n:
            ...
            self.further_num += 1
            data_output = open('../../data/results/EXP_Pairing/further_optim_'+self.matrix_type + str(self.b)+'.txt', 'wb')
            pickle.dump(self.UPPER, data_output)
            data_output.close()
            print('n:{}; further_num:{}'.format(self.n, self.further_num))
            return
