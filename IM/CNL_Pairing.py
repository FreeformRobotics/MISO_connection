...
class CNL_Pairing():
    ...
    def CNL_pair(self, start_csg):
        ...
        for child in self.A.nodes[start_csg.Match[0]].children:
            A_CNL_sorted.append((self.A.nodes[start_csg.Match[0]].CNL[child.parent_C], child.ID))
        B_CNL_sorted=[]
        for child in self.B.nodes[start_csg.Match[1]].children:
            B_CNL_sorted.append((self.B.nodes[start_csg.Match[1]].CNL[child.parent_C], child.ID))

        def takeFirst(elem):
            return elem[0]
        #For each CN in CNL, sort from largest to smallest
        A_CNL_sorted.sort(key=takeFirst, reverse=True)
        B_CNL_sorted.sort(key=takeFirst, reverse=True)

        for Aterm in reversed(A_CNL_sorted):#"reversed" is for "romove" to avoid destroying the traversability of the "for" loop
            for Bterm in reversed(B_CNL_sorted):
                if Aterm[0]==Bterm[0]:
                    #Automatic matching of the nodes with the same CN
                    globals()['cnl_csg'+str(self.j)]=AnyNode(ID=self.j, name='csg'+str(self.j), parent=start_csg, Match=(Aterm[1],Bterm[1]))
                    self.j += 1
                    A_CNL_sorted.remove(Aterm)
                    B_CNL_sorted.remove(Bterm)
                    break
        #Match CN in descending order.
        for i in range(min(len(A_CNL_sorted),len(B_CNL_sorted))):
            globals()['cnl_csg' + str(self.j)] = AnyNode(ID=self.j, name='csg' + str(self.j), parent=start_csg, Match=(A_CNL_sorted[i][1], B_CNL_sorted[i][1]))
            self.j += 1
    ...
    def Virtual_roots(self, printer=True):
        ...
        while len(A_index_lef)!=0:
            ...
            self.CNL_pair(globals()['cnl_csg' + str(self.n +self.vrm_i)])
            if printer==True:
