...
for n in p(range(4, N)):
    ...
    data_input = open('/data/results/EXP_Pairing/c_'+str(n)+'.txt', 'rb')
    ...
    result[instance_pointer]=D
    elapsed=time.time()-start
    encode_times.append(elapsed)


data_output = open('/data/results/library_linear.txt', 'wb')
pickle.dump(result, data_output)
data_output.close()

data_output = open('/data/results/encode_times_linear.txt', 'wb')
