    ...
    def loop_tree(self, parent_ID):
        ...
        if len(child_list)!=0:
            for child in child_list:
                ...
                globals()['v' + str(child)] = AnyNode(name='v' + str(child), parent=globals()['v' + str(parent_ID)], ID=child, CNL=[], pointer=[], parent_C=None)
    ...
    def matrix2tree(self, printer=True):
        ...
        for i in range(np.shape(self.matrix)[0]):
            ...
            if np.linalg.norm(self.matrix[i,:])==0:
                self.roots.append(i)
                self.allV.remove(i)
                globals()['v'+str(i)]=AnyNode(name='v'+str(i), ID=i, CNL=[], pointer=[], parent_C=None)
                self.loop_tree(i)
        ...
        while len(self.allV)!=0:
            ...
            while j not in circuit:
                ...
                self.matrix[j,:]=0
                globals()['v'+str(j)]=AnyNode(name='v'+str(j), ID=j, CNL=[], pointer=[], parent_C=None)
                self.loop_tree(j)
                ...
                for B in Bifurcation:
                    ...
                    for child in B.children:
                        if child.pointer==[]:
                            Active=False
                    if Active==True:
                        B.pointer.append(sorted(B.CNL, reverse=False))
                        ...
                        for dup in sorted(list_duplicates(B.CNL), reverse=False):
                            if len(dup[1]) == 1:
                                for child in B.children:
                                ...
                                Current_number.sort(key=lambda elem:elem[0], reverse=False)
